...
function LeadsPanel() {
  ...
  return <><div className="filter-panel"><span className="result-count">{rows.length} enquiries</span><select className="input compact-input" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="">All statuses</option><option value="new">New</option><option value="contacted">Contacted</option><option value="converted">Converted</option><option value="closed">Closed</option></select></div><section className="panel"><Table rows={rows} empty="No enquiries found." columns={[{ key: 'name', label: 'Lead', render: (row) => <Link className="table-person table-link" to={`/admin/operations/leads/${row.id}`}><span className="avatar small">{row.name?.slice(0, 1)}</span><span><strong>{row.name}</strong><small>{row.email || row.phone}</small></span></Link> }, { key: 'service', label: 'Service', render: (row) => row.service || 'General enquiry' }, { key: 'created_at', label: 'Received', render: (row) => dateLabel(row.created_at) }, { key: 'status', label: 'Status', render: (row) => <select className="input compact-input" value={row.status || 'new'} onChange={(event) => update(row.id, event.target.value)}><option value="new">New</option><option value="contacted">Contacted</option><option value="converted">Converted</option><option value="closed">Closed</option></select> }]} /></section></>;
}
