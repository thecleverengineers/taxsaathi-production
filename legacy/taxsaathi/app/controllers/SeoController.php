...
final class SeoController extends Controller
{
    ...
    public function generateFromServices(): void
    {
        ...
        redirect('admin/seo?target_type=service&status=1');
    }
    ...
    public function sitemap(): void
    {
        ...
        $staticPages = [
            ...
            ['loc' => $baseUrl . '/services', 'priority' => '0.95', 'changefreq' => 'daily'],
            ['loc' => $baseUrl . '/tax-calculators', 'priority' => '0.80', 'changefreq' => 'weekly'],
            ['loc' => $baseUrl . '/about', 'priority' => '0.70', 'changefreq' => 'monthly'],
            ['loc' => $baseUrl . '/contact', 'priority' => '0.70', 'changefreq' => 'monthly'],
            ['loc' => $baseUrl . '/service-order', 'priority' => '0.70', 'changefreq' => 'weekly'],
            ['loc' => $baseUrl . '/terms-conditions', 'priority' => '0.45', 'changefreq' => 'monthly'],
            ['loc' => $baseUrl . '/privacy-policy', 'priority' => '0.45', 'changefreq' => 'monthly'],
            ['loc' => $baseUrl . '/refund-policy', 'priority' => '0.45', 'changefreq' => 'monthly'],
        ];
        ...
        foreach ($this->activeSeoServices() as $service) {
            ...
            $urls[] = [
                ...
                'lastmod' => $this->seoLastModified($service),
                'changefreq' => 'weekly',
                'priority' => ((int) ($service['is_featured'] ?? 0) === 1) ? '0.92' : '0.85',
            ];
        }

        foreach ($this->activeSeoCategories() as $category) {
            $identifier = trim((string) ($category['slug'] ?? '')) !== ''
            ...
            $urls[] = [
                'loc' => $baseUrl . '/service-category?category=' . rawurlencode($identifier),
                'lastmod' => $this->seoLastModified($category),
                'changefreq' => 'weekly',
                'priority' => '0.78',
            ];
        }

        $this->outputSeoUrlSet($urls);
    }

    public function sitemapServices(): void
    {
        $urls = [];
        $baseUrl = rtrim((string) base_url('/'), '/');
    ...
    public function robots(): void
    {
        ...
        echo "Disallow: /partner/\n";
        echo "Disallow: /executive/\n";
        echo "Disallow: /auth\n";
        echo "Disallow: /logout\n";
        echo "Disallow: /service-order/payment\n";
        echo "Disallow: /*?application_id=\n";
        echo "Disallow: /*?next_step=\n\n";
        echo "Sitemap: " . $baseUrl . "/sitemap.xml\n";
        echo "Sitemap: " . $baseUrl . "/sitemap-services.xml\n";
    }

    private function outputSeoUrlSet(array $urls): void
    {
        ...
        foreach ($urls as $url) {
            ...
            $lastmod = htmlspecialchars((string) ($url['lastmod'] ?? date('Y-m-d')), ENT_XML1 | ENT_QUOTES, 'UTF-8');
            $changefreq = htmlspecialchars((string) ($url['changefreq'] ?? 'weekly'), ENT_XML1 | ENT_QUOTES, 'UTF-8');
            $priority = htmlspecialchars((string) ($url['priority'] ?? '0.80'), ENT_XML1 | ENT_QUOTES, 'UTF-8');
    ...
    private function activeSeoServices(): array
    {
        try {
    ...
    private function activeSeoCategories(): array
    {
        try {
            return app('db')->fetchAll(
    ...
    private function seoLastModified(array $row): string
    {
        $raw = trim((string) ($row['updated_at'] ?? $row['created_at'] ?? ''));
    ...
    private function authorizeSeoAdmin(): void
    {
        if (function_exists('can')) {
            foreach (['services.manage', 'website.manage', 'content.manage', 'admin.services.manage', 'admin.content.manage'] as $permission) {
                try {
                    if ((bool) can($permission)) {
                        return;
                    }
                } catch (\Throwable $e) {
                    // Continue to the default permission gate below.
                }
            }
        }
